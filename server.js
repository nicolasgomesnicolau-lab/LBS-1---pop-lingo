...
const KARAOKE_NGROK = process.env.KARAOKE_NGROK || 'https://lbs-1-api-jarvis-for-pop-lingo.onrender.com';
...
// Get video info (title + duration) via YouTube Data API
  ...
  if (method === 'GET' && url.pathname === '/api/achievements') {
    ...
    supabase.auth.getUser(token).then(function(result) {
      ...
      supabase.from('achievements_data').select('data').eq('user_id', user.id).single().then(function(r) {
  ...
  if (method === 'POST' && url.pathname === '/api/achievements/save') {
    readBody(req).then(function(body) {
      var data = JSON.parse(body);
      var token = data.token || '';
      ...
      supabase.auth.getUser(token).then(function(result) {
        ...
        supabase.from('achievements_data').upsert({ user_id: user.id, data: achievements, updated_at: new Date().toISOString() }, { onConflict: 'user_id' }).then(function(r) {
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: !r.error }));
        });
      });
  ...
  // Playlist API (per-user music playlist)
  if (method === 'GET' && url.pathname === '/api/playlist') {
    ...
    supabase.auth.getUser(token).then(function(result) {
      ...
      supabase.from('playlist_data').select('data').eq('user_id', user.id).single().then(function(r) {
        var data = (r.data && r.data.data) || [];
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ data: data }));
      });
  ...
  if (method === 'POST' && url.pathname === '/api/playlist/save') {
    readBody(req).then(function(body) {
      var data = JSON.parse(body);
      var token = data.token || '';
      ...
      supabase.auth.getUser(token).then(function(result) {
        ...
        supabase.from('playlist_data').upsert({ user_id: user.id, data: playlist, updated_at: new Date().toISOString() }, { onConflict: 'user_id' }).then(function(r) {
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: !r.error }));
        });
      });
