...
const MoviesTab = (() => {
  ...
  function createMoviePlayer() {
    ...
    if (!youtubeApiReady || !currentClip || currentView !== 'player') return;

    const videoId = getYoutubeId(currentClip.videoUrl);
    if (!videoId) return;

    // Ensure the player element exists and is empty
    const playerContainer = document.querySelector('.movie-video-container');
    if (!playerContainer) {
      setTimeout(createMoviePlayer, 100); // Retry if container not ready
      return;
    }
    playerContainer.innerHTML = '<div id="movie-youtube-player"></div>'; // YouTube API will replace this div
    ...
    movieYTPlayer = new YT.Player('movie-youtube-player', {
      ...
      playerVars: {
        ...
        'modestbranding': 1,
        'enablejsapi': 1,
        'start': Math.floor(movieCurrentTime),
      },
      events: {
  ...
  function onPlayerStateChange(event) {
    if (event.data === YT.PlayerState.PLAYING) {
      ...
      stopMovieSync();
      movieCurrentTime = 0;
      // Optional: go back to list view or play next clip
      // currentView = 'list';
      // App.refreshCurrentTab();
    }
    updateMoviePlayerUI();
  }
  // --- End YouTube API Callbacks ---
  ...
  function startMovieSync() {
    stopMovieSync();
    // Only start polling if player is playing
    if (movieYTPlayer && movieYTPlayer.getPlayerState() === YT.PlayerState.PLAYING) {
  ...
  function syncMovieTime() {
    ...
    updateMoviePlayerUI(); // Update seekbar and time displays continuously
  }
  ...
  function updateMoviePlayerUI() {
    ...
    var playBtn = document.getElementById('movie-play-btn');
    if (playBtn) {
    ...
    var seekbar = document.getElementById('movie-seekbar');
    var curEl = document.getElementById('movie-time-current');
    var totEl = document.getElementById('movie-time-total');
    ...
    if (totEl) totEl.textContent = formatTime(moviePlayerDuration);
  }
    ...
    return `
      <div class="movie-player-view active">
        ...
        <div class="movie-video-container">
          <div id="movie-youtube-player"></div> <!-- Placeholder for YouTube API -->
        </div>
  ...
  function bindEvents(container) {
      ...
      el.addEventListener('click', () => {
        ...
        if (series) {
          ...
          if (clip) {
            currentClip = clip;
            currentView = 'player';
            movieCurrentTime = 0; // Reset time when loading new clip
            moviePlayerDuration = 0;
            movieIsPlaying = false;
            // Player will be created by onYouTubeIframeAPIReady or createMoviePlayer directly
            App.refreshCurrentTab(); 
          }
        }
      });
    ...
    // These listeners are set *after* the player HTML is rendered by renderPlayer()
    // and only if currentClip is set (meaning we are in player view)
    if (currentView === 'player' && currentClip) {
      // Create player if API is ready
      if (youtubeApiReady) {
        createMoviePlayer();
      }
      ...
      container.querySelector('#movie-skip-back')?.addEventListener('click', () => {
        if (movieYTPlayer) {
          movieYTPlayer.seekTo(movieCurrentTime - 10, true);
        }
      });
      ...
      container.querySelector('#movie-seekbar')?.addEventListener('input', function() {
        if (movieYTPlayer && moviePlayerDuration > 0) {
          const val = (this.value / 100) * moviePlayerDuration;
          document.getElementById('movie-time-current').textContent = formatTime(val);
        }
      });

      container.querySelector('#movie-seekbar')?.addEventListener('change', function() {
        if (movieYTPlayer && moviePlayerDuration > 0) {
          const seekToTime = (this.value / 100) * moviePlayerDuration;
          movieYTPlayer.seekTo(seekToTime, true);
        }
      });
    }
    // --- End Player controls ---
  ...
  // Expose onYouTubeIframeAPIReady to the global scope
  window.onYouTubeIframeAPIReady = () => MoviesTab.onYouTubeIframeAPIReadyInternal();

  return { 
    render, 
    bindEvents,
    onYouTubeIframeAPIReady: onYouTubeIframeAPIReady // Expose YouTube API ready callback
  };
})();
      <div class="tab-header">
        <div class="tab-header-icon">🎬</div>
